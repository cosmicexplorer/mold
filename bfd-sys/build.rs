...
fn main() {
  ...
  let bfd_inc_dir: PathBuf =
    [complete_build_dir.as_path(), Path::new("bfd")]
      .iter()
      .collect();
  ...
  let build_path = complete_build_dir.as_path();
  add_static_build_lib(build_path, "bfd", "bfd");

  add_static_build_lib(build_path, "zlib", "z");

  add_static_build_lib(build_path, "intl", "intl");

  add_static_build_lib(build_path, "libiberty", "iberty");

  add_static_build_lib(build_path, "opcodes", "opcodes");
